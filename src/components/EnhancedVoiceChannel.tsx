...
import {
  ...
  VideoTileInfo,
  VoiceRosterMember,
  MediaState as ManagerMediaState,
...
const EnhancedVoiceChannel: React.FC<EnhancedVoiceChannelProps> = ({
        ...
        try {
          ...
          if (isMounted) {
            ...
            if (hasAnyPerms) {
              if (!permissions.audio && permissions.video) {
                setPermissionError(
                  "Video-only mode: Microphone access denied. You can still use video features."
                );
              } else if (permissions.audio && !permissions.video) {
                setPermissionError(
                  "Audio-only mode: Camera access denied. You can still use voice features."
                );
              }
            }
          }
          ...
          if (isMounted) {
            ...
            if (manager && manager.hasAnyPermissions()) {
              ...
              if (!permissions.audio && !permissions.video) {
                setPermissionError(
                  "No audio or video permissions available. Please allow access and try again."
                );
              ...
              if (
                ...
                setPermissionError(
                  "Camera and microphone access denied. Please click the camera/microphone icon in your browser address bar and allow permissions, then refresh the page."
                );
                ...
                setPermissionError(
                  "Camera/microphone constraints could not be satisfied. Try refreshing the page."
                );
              } else if (error?.name === "SecurityError") {
                setPermissionError(
                  "Access denied due to security restrictions. Please ensure you are on a secure (HTTPS) connection."
                );
              } else {
                setPermissionError(
                  `Media access error: ${error?.message || "Unknown error"}. Please refresh the page and try again.`
                );
              }
            }
          }
      ...
      manager.onVoiceRoster((members: any[]) => {
        if (isMounted) {
          ...
          const voiceParticipants: Participant[] = remoteMembers.map(
            (member) => {
              const odattendeeId = String(
                member.odattendeeId || member.attendeeId || member.id || ""
              );
              const oduserId = String(
                member.oduserId ||
                  member.userId ||
                  member.user_id ||
                  member.name ||
                    ...
                    screenSharing:
                      existing.screenTileId !== undefined
                        ? vp.mediaState.screenSharing ||
                          existing.mediaState.screenSharing
                        : vp.mediaState.screenSharing,
                  },
                };
              }
          ...
          setParticipants((prev) =>
            prev.map((p) =>
              p.id === attendeeId
                ? {
                    ...p,
                    mediaState: { ...p.mediaState, screenSharing: isSharing },
                  }
                : p
            )
          );
          ...
          if (tile.attendeeId) {
            ...
            setParticipants((prev) => {
              const existingIndex = prev.findIndex(
                (p) => p.id === baseAttendeeId || p.id === tile.attendeeId
              );
  ...
  const handleRetryAudioOnly = async () => {
    ...
    try {
      ...
      setIsInitializing(false);
      setPermissionError(
        `Audio access error: ${error.message || "Unknown error"}`
      );
    }
  };
  ...
  const handleRetryVideoOnly = async () => {
    ...
    try {
      ...
      setIsInitializing(false);
      setPermissionError(
        `Video access error: ${error.message || "Unknown error"}`
      );
    }
  };
